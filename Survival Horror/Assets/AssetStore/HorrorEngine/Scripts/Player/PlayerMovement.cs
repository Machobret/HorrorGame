...
namespace HorrorEngine
{
    ...
    public class PlayerMovement : MonoBehaviour, IDeactivateWithActor
    {
        ...
        private void UpdateMovement()
        {
            ...
            float fwdContribution = Mathf.Abs(Vector3.Dot(transform.forward, fwdMove.normalized));
            float rightContribution = Mathf.Abs(Vector3.Dot(transform.right, rightMove.normalized));
        ...
        Vector3 GetForwardMovement(out float speed)
        {
            ...
            else
            {
                if (fwd > Mathf.Epsilon)
                {
                    speed = m_Running && DepleteStamina(1f) ? m_MovementRunSpeed : m_MovementSpeed * (m_MovementInputType == MovementInputType.Analog ? absFwd : 1f);
                }
                else if (fwd < -Mathf.Epsilon)
                {
                    speed = m_MovementBackwardsSpeed * (m_MovementInputType == MovementInputType.Analog ? absFwd : 1f);
                }
            }
        ...
        Vector3 GetRightMovement(out float speed)
        {
            ...
            if (right > Mathf.Epsilon || right < -Mathf.Epsilon)
                speed = m_MovementLateralSpeed * (m_MovementInputType == MovementInputType.Digital ? 1f : absRight);
