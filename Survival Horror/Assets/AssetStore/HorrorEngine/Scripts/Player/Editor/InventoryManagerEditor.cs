...
namespace HorrorEngine
{
    public class InventoryManagerEditor : EditorWindow
    {
        ...
        void DrawDocuments(Inventory inventory)
        {
            ...
            m_Document = EditorGUILayout.ObjectField(new GUIContent("Add Document"), m_Document, typeof(DocumentData), false) as DocumentData;
            if (m_Document != null)
            {
                if (GUILayout.Button("Add Document"))
                {
                    inventory.Documents.Add(m_Document);
                }
            }
        }
    }
