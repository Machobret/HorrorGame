...
namespace HorrorEngine
{
    ...
    public class SettingsManager
    {
        ...
        public bool GetInt(SettingsElementContent Key, out int outValue)
        {
            ...
            if (!int.TryParse(outValueStr, NumberStyles.Integer, CultureInfo.InvariantCulture, out outValue))
            {
                if (bool.TryParse(outValueStr, out bool boolValue))
                {
                    outValue = boolValue ? 1 : 0;
                    m_Settings[Key] = outValue.ToString(CultureInfo.InvariantCulture);
                    return true;
                }

                string defaultValue = Key.GetDefaultValue();
                Debug.LogWarning($"Invalid value '{outValueStr}' for setting '{Key.SettingsKey}'. Resetting it to '{defaultValue}'.");

                if (!int.TryParse(defaultValue, NumberStyles.Integer, CultureInfo.InvariantCulture, out outValue))
                {
                    outValue = 0;
                    return false;
                }
